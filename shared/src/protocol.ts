 ...
 *
 * A parent's `state` describes its OWN turn: when Claude Code dispatches eight
 * subagents and its turn then closes, the parent is genuinely `idle` while the
 * children keep working. That is a second axis, not a correction to the first,
 * and without it the deck says "idle" for half an hour of real work.
 *
 * The daemon's `SubagentTimelineTracker` is the source — it counts the
 * `SubagentStart`/`SubagentStop` hook pairs directly. It is deliberately NOT
 * derived from timeline rows: those are dropped by dedup and shed first by
 * eviction, so a timeline-derived count reads zero in exactly the fan-out case
 * it exists to describe.
 *
 * Children never become `SessionInfo` entries of their own. They are not
 * separately steerable, they would flood a deck sized for sessions, and a
 * device pressing one has nothing to press it with.
 */
export interface SubagentSummary {
  /** Children currently running (started, no stop seen). */
  active: number;
  /** Highest concurrent `active` since the last time it reached zero — the
   *  width of the current fan-out, which `active` alone loses as children
   *  drain one by one. */
  peak: number;
...
export interface SessionInfo {
   ...
   *
   *  Emitted for every session that has EVER had a child, including once they
   *  all finish (`active: 0`). Absent means "no child was ever seen here", not
   *  "zero right now": clients merge retain-on-absent, so dropping the field
