...
- **Passive session observation** scans native processes through `Get-CimInstance Win32_Process` (POSIX uses `ps`; git-bash `ps.exe` only sees MSYS processes, so it was never a real backend). Standalone Claude Code sessions are expected to surface from that scan plus `%USERPROFILE%\.claude\sessions\` — both the native `claude.exe` and the npm install, whose process is `node.exe …\@anthropic-ai\claude-code\cli.js` with the binary name nowhere in argv (Windows verification pending). **Codex sessions do surface on Windows, through the `codex_*` lifecycle hooks** (`HookCodexSessions`), which need neither `ps` nor `lsof` — install them with `agentdeck daemon install`. What the Windows scan cannot add to those rows is the rollout-derived enrichment (model, context %, current tool, and the CLI/desktop split): mapping a pid to its open rollout file needs `lsof`/procfs, which Windows lacks. Tracked in [#143](https://github.com/puritysb/AgentDeck/issues/143); a session-id-keyed rollout lookup would close it without either tool. Observed OpenCode/Antigravity rows also carry no `cwd` there (no working directory in `Win32_Process`), so they label by agent name rather than project.
