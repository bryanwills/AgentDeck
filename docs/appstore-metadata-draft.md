...
```
터미널을 계속 지켜보지 않아도 됩니다. Claude Code, Codex, OpenCode, OpenClaw 세션의 상태·도구 호출·사용량을 잔잔한 한 화면에서 — 당신이 필요한 순간엔 바로 알려드립니다. 하드웨어 없이 Mac 한 대로 시작하세요.
```
...
```
ターミナルを見張り続ける必要はありません。Claude Code、Codex、OpenCode、OpenClawセッションの状態・ツール呼び出し・使用量を静かな1画面で。呼ばれた瞬間だけ、そっと知らせます。ハードウェアなしでMac1台で始められます。
```
...
```
Stop watching terminals. Every Claude Code, Codex, OpenCode, and OpenClaw session on one calm screen, and a ping the moment you're needed. No hardware required.
```
