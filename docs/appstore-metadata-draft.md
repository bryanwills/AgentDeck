...
```
코딩 에이전트와 대화만 하지 마세요 — Mac에서 Claude Code, Codex, OpenCode, OpenClaw 세션을 실시간으로 조종하세요. 도구 호출·사용량·APME 평가를 한 화면에서, 하드웨어 없이도 17개 디스플레이 프리뷰까지.
```

**iOS App**

```
Mac에서 조종 중인 세션을 iPad·iPhone에서도 놓치지 마세요. Claude Code, Codex, OpenCode, OpenClaw 상태·도구 호출·사용량·APME 점수를 실시간으로. 같은 Wi-Fi의 Mac에서 AgentDeck 실행이 필요합니다.
```
...
```
AgentDeck는 대화 말고 조종하게 해주는 앱입니다. 터미널 안에 갇혀 있던 Claude Code, Codex, OpenCode, OpenClaw 세션을 화면 하나로 꺼내 놓고 직접 조종하세요. 제출 앱에 내장된 샌드박스 Swift daemon이 세션 감시, 훅 이벤트, 로컬 네트워크 페어링을 로컬에서 직접 처리합니다.

"Stop Chatting. Start Steering."

Claude Code를 비롯한 AI 에이전트를 터미널에서 돌릴 때, 세션 진행 상태 · 호출한 도구 · 토큰 사용량 · 평가 점수 같은 핵심 정보가 화면 하나에 다 들어옵니다. 작업 중인 터미널 창에 집착할 필요 없이, 에이전트를 "대화" 상대가 아니라 "조종" 대상으로 다루세요.
...
• iPad/iPhone 컴패니언 (무료)
Bonjour 자동 발견 + QR 페어링. Mac에서 조종 중인 세션을 침대 머리맡 iPad에서도 그대로 지켜보세요.
...
하드웨어가 없어도 핵심 기능을 바로 조종할 수 있습니다.
...
AgentDeck는 독립적인 프로젝트이며 Anthropic, OpenAI, Google, SST, Corsair/Elgato, DIVOOM, Ulanzi, Waveshare 및 언급된 기타 제3자와 제휴, 후원, 또는 승인 관계가 없습니다. Claude™, Claude Code™, Codex™, Stream Deck®, Pixoo® 등 모든 상표는 각 소유자의 자산입니다. 자세한 상표 고지와 오픈소스 라이선스: https://github.com/puritysb/AgentDeck/blob/master/ATTRIBUTION.md
```

**iOS App**

```
AgentDeck(iOS)는 Mac에서 실행 중인 AgentDeck과 페어링해, 조종 중인 세션을 실시간으로 그대로 보여주는 컴패니언 앱입니다. 독립 실행형 앱이 아니며, 같은 Wi-Fi에 연결된 Mac에서 AgentDeck이 실행 중이어야 합니다.

"Stop Chatting. Start Steering."

Mac에서 Claude Code 등 AI 에이전트를 조종할 때, 세션 진행 상태 · 호출한 도구 · 토큰 사용량 · 평가 점수 같은 핵심 정보를 iPad·iPhone 화면에서 실시간으로 확인하세요.
...
```
🎉 AgentDeck, 첫 App Store 출시 — 대화 말고 조종하세요.
...
```
コーディングエージェントと会話するだけじゃもったいない — MacでClaude Code、Codex、OpenCode、OpenClawセッションをリアルタイムに操縦。ツール呼び出し・使用量・APME評価を1画面で、ハードウェアなしでも17種のデバイスプレビューまで。
```

**iOS App**

```
Macで操縦中のセッションをiPad・iPhoneでも見逃さない。Claude Code、Codex、OpenCode、OpenClawの状態・ツール呼び出し・使用量・APMEスコアをリアルタイムに。同一Wi-Fi上のMacでAgentDeckの実行が必要です。
```
...
```
AgentDeckは、会話するのではなく操縦させてくれるアプリです。ターミナルに閉じ込められていたClaude Code、Codex、OpenCode、OpenClawのセッションを1つの画面に連れ出し、直接操縦しましょう。提出されたアプリに内蔵されたサンドボックス化されたSwift daemonが、セッション監視・フックイベント・ローカルネットワークペアリングをローカルで直接処理します。

"Stop Chatting. Start Steering."

ターミナルでClaude CodeなどのAIエージェントを実行する際、セッションの進行状況・呼び出したツール・トークン使用量・評価スコアといった重要な情報が1画面にまとまります。作業中のターミナルウィンドウに縛られる必要はありません。エージェントを「会話する」相手ではなく「操縦する」対象として扱いましょう。
...
Bonjour自動検出 + QRペアリング。Macで操縦中のセッションを、ベッドサイドのiPadからもそのまま見守れます。
...
• 17種のデバイスプレビューギャラリー
ハードウェアがなくても、Stream Deck / Stream Deck+ / D200H / iPad / InkDeck e-ink / ESP32ボード / Pixoo / Timebox / iDotMatrix / TUIなど、Swiftアプリの17種類のディスプレイプレビューを確認できます。
...
```
AgentDeck（iOS）は、Macで動作するAgentDeckとペアリングし、操縦中のセッションをそのままリアルタイムに映し出すコンパニオンアプリです。単独動作するアプリではなく、同一Wi-Fi上のMacでAgentDeckが実行中である必要があります。

"Stop Chatting. Start Steering."

MacでClaude CodeなどのAIエージェントを操縦する際、セッションの進行状況・呼び出したツール・トークン使用量・評価スコアといった重要な情報を、iPad・iPhone画面でリアルタイムに確認できます。
...
```
🎉 AgentDeck、初のApp Storeリリース — 会話じゃなく、操縦を。
...
```
Chatting only gets you so far — steer live Claude Code, Codex, OpenCode, and OpenClaw sessions on Mac + iPad. Tools, usage, APME scores, plus 17 display previews.
```

**iOS App**

```
Keep steering from anywhere — see your Mac's Claude Code, Codex, OpenCode, and OpenClaw sessions live on iPad or iPhone. Requires AgentDeck running on your Mac.
```
...
```
AgentDeck Dashboard lets you steer instead of just chatting. It pulls Claude Code, Codex, OpenCode, and OpenClaw sessions out of the terminal and onto one screen you can actually steer. Its sandboxed, built-in Swift daemon handles session tracking, hook events, and local pairing.

"Stop Chatting. Start Steering."

Session progress, tool calls, token usage, and evaluation scores — all in one place, so you're not stuck staring at a terminal window. Treat your agent as something you steer, not something you chat with.
...
Auto-discovers your Mac over Wi-Fi via Bonjour. QR pairing fallback for different-subnet setups. Keep steering from a second screen without alt-tabbing.
...
None of the hardware is required — you can start steering on a stock Mac.
...
```
AgentDeck for iOS is a companion that pairs with AgentDeck running on your Mac and mirrors the session you're steering, live. It is not a standalone app — AgentDeck must be running on a Mac on the same Wi-Fi network.

"Stop Chatting. Start Steering."

When you're steering Claude Code or another AI agent on your Mac, see session progress, tool calls, token usage, and evaluation scores live on your iPad or iPhone.
...
• Mac pairing (free)
Auto-discovers your Mac over Wi-Fi via Bonjour. QR pairing fallback for different-subnet setups. Keep steering visible from a second screen without alt-tabbing.
...
```
🎉 AgentDeck Dashboard — first App Store release. Stop chatting, start steering.
