...
| APME says `better-sqlite3 native binding unavailable` | Dependencies were installed with a different Node runtime/ABI than the daemon uses, or installation ran on unsupported Node 20/odd-numbered Node | Run `agentdeck diag native` to see the exact executable and ABI. Then switch to Node 22, 24, or 26 and run `npx @agentdeck/setup --yes`; setup reinstalls the binding and re-registers daemon autostart with that same runtime |
