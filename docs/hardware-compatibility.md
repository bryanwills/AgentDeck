...
| RockBase NM-EPD-420 | ESP32 e-ink | ESP32-S3 N16R8 | GDEY042Z98 tri-color e-ink · 4.2″ · 400×300 | USB serial · Wi-Fi WS | Yes |
| LilyGo T5 ePaper S3 | ESP32 e-ink | ESP32-S3 N16R8 | ED047TC2 grayscale e-ink · 4.7″ · 960×540 | USB serial · Wi-Fi WS | Yes |
...
| RockBase NM-EPD-420 standard SKU | `nm_epd_420` · `nm_epd_420` | ESP32-S3-WROOM-1-N16R8 | 16 MB · 8 MB | 4.2″ tri-color e-ink · 400×300 landscape | GDEY042Z98 | BOOT GPIO0 · USER GPIO45 · no touchscreen controller | ES8311 codec · mic · speaker · Li-ion path | Native USB JTAG | Single 3 MB app | Shipping |
| LilyGo T5 ePaper S3 V2.4 | `lilygo_epd47` · `lilygo_epd47` | ESP32-S3-WROOM-1-N16R8 | 16 MB · 8 MB | 4.7″ grayscale e-ink · 960×540 landscape | ED047TC2 parallel/I2S | GT911 two-point touch at 0x5D, attached by an 8-pin 0.5 mm FPC to header P6 (VDD3V3/GND/SDA18/SCL17/INT47/T_RST) · User GPIO21 · BOOT GPIO0 · RST | PCF8563 RTC · microSD · Li-Po JST-PH 2.0 | Native USB JTAG | 6.25 MB | Shipping |
...
### E-ink control and audio capability

This is the six-field hardware decision table used by the face contract. A
button being electrically wake-capable does not mean the current push firmware
must enter deep sleep.

| Model · panel geometry / direction | MCU · PSRAM | Power | Button deep-sleep wake | Microphone path | Speaker amplifier enable |
|---|---|---|---|---|---|
...
| RockBase NM-EPD-420 · GDEY042Z98 400×300 landscape, black/white/red | ESP32-S3-WROOM-1-N16R8 · 8 MB | USB-C or Li-ion | **BOOT GPIO0 yes**; USER GPIO45 no | ES8311 full-duplex I2S: MCLK21, BCLK15, LRCLK17, ADC→ESP16, ESP→DAC18; codec enable GPIO44. Codec **probed present at 0x18** (2026-08-30); capture still gated on a paper-native listening indicator | **GPIO41** · playback **audible on hardware** (2026-08-31). Output level is an amplifier property: `BOARD_SPK_DEFAULT_VOLUME` = 100 here, because the shared default of 70 is -18 dB and inaudible on this Class-D stage |
...
rewriting the identical application as DIO recovered it immediately. The
`qio_opi` PlatformIO memory type names the octal PSRAM wiring and does not make
the SPI flash transport QIO.
...
This table stays the source of truth for the **human** columns above — panel, controller, input, peripherals, status. The **machine** fields a build needs (chip family, flash size and mode, bootloader offset, upload baud, esptool flags, OTA capability, CLI aliases) live in [`shared/src/esp32-boards.ts`](../shared/src/esp32-boards.ts), which also generates the `esp32-v*` release matrix. Two sources over disjoint columns, bound by one gate: `node scripts/generate-esp32-board-matrix.mjs --check` fails when a Shipping row here has no board there (that board would ship no firmware and nothing else would fail — how `t_embed`, `t_display_pro` and `esp32_c6_147` had no binaries at 1.0.1), or when a board's chip, flash size, baud or upload flags disagree with `esp32/platformio.ini`, `bridge/src/cli.ts` or the alias table in [esp32.md](esp32.md). That file also carries `webFlash` — whether the **browser** flasher offers a board — which is an evidence field, not a capability one: it is true only for boards a hardware run measured, and `agentdeck esp32 flash` writes every board here regardless. See [esp32.md § Flash over USB](esp32.md#flash-over-usb).
...
- **The camera probe also selects the S3-Pro's screen orientation, so the two rows above run different UIs from one binary.** `Camera::init()` runs before display bring-up; a unit that answers comes up **portrait** in the Pocket UI (SESSIONS / CAM / USAGE, LVGL pointer widgets), and a unit that does not comes up **landscape** in the Ticker UI (Focus / Usage / Sessions, rocker + swipe). The desk-mounted no-camera unit uses landscape rotation 3 so its USB-C cable enters from the left; its CST226SE coordinates use the matching rotation-3 transform. Both report `t_display_pro` and take the same OTA image, so a board id alone does not tell you which layout a given unit is showing — read the reported orientation, not the board string. The camera unit's stack is portrait-native (panel GRAM, CST226SE touch reporting, shield mounting), which is why that path needs no rotation compensation.
- The on-hand NM-EPD-420 uses the GDEY042Z98 controller's native landscape direction (rotation 0). **Do not use SSD1683 `refresh_bw()` on this standard tri-color SKU.** A full-screen trial first caused severe flashing and a dark wash; a second trial restricted to the byte-aligned, red-free `(8,56) 392×244` window still lifted black pigment across the physical panel, producing a uniform gray veil over otherwise readable content. The driver's `hasPartialUpdate` describes RAM addressing, not compatibility of the experimental B/W waveform with this glass. AgentDeck therefore has no partial-refresh code or B/W framebuffer for NM: every admitted repaint uses the stock full black/white/red waveform, with one extended waveform at boot to recondition both pigment planes. Red remains useful in the fixed top brand rail and DECISION action rail. Automatic content changes are coalesced for five minutes; a deliberate physical-key action still updates immediately. This is slower than a CFA color reader but is the only observed stable mode for this panel.
- LilyGo EPD47 treats a hard refresh as a real `epd_clear()` followed by the new grayscale image. It runs before the first frame, after four differential replacements, or after ten minutes. This removes both shipped factory artwork and residue that quiet frame replacement can leave behind.
- **EPD47 content changes erase the previous frame; only the scheduled pigment reset clears.** One tab tap cost exactly one `epd_clear()`, and one clear on this driver is several whole-panel inversions — a single tap read as a storm of flashes. The canvas retains the frame it last wrote (a second 4-bit PSRAM framebuffer, optional; without it the surface degrades to the clearing path) and every admitted content change drives that frame back to paper with `epd_draw_image(WHITE_ON_WHITE)` before drawing the next one. This is required by the pinned driver, whose grayscale draw contract assumes a white surface. Differential replacements increment their own bounded count and never reset the hard-clear age; only a complete hard anti-ghost clear advances `fullRefreshCount`. A body-only hard clear is deliberately not used because the following full-frame draw would redraw an uncleared header and violate the same white-surface precondition. The executable policy is `epd47_refresh_policy.h`, shared with host tests.
- **The autonomous tab is hysteretic, because a page change still costs a retained-frame erase plus a complete draw.** `automaticPage()` reads `attention + processing`, a count that oscillates continuously on a working machine, and following it directly made **88% of all EPD47 repaints hard clears** before retained-frame erasing existed (measured 2026-08-30: 713/639, then 754/669 twenty-seven minutes later). `AgentDeckEpd47::arbitratePage` now requires a newly-correct page to hold for two repaint windows before it is adopted, and a settled swap rides the normal cadence instead of forcing an immediate repaint. Attention is exempt and still switches at once. Host-tested in `esp32/sim/tests/`.
- **EPD47 is the only e-ink panel in the fleet with grayscale, and the ink set collapses at the source for the others.** Its ED047TC2 carries a 4-bit framebuffer that until 2026-08-30 held two of its sixteen levels. Greys are RGB565 values decoded from the 6-bit green channel by both the firmware canvas and the host simulator. GxEPD2 maps *every* non-white colour to solid ink, so on the 1-bit InkDeck and the tri-color NM the grey constants resolve to plain black/white rather than being left to the backend — a `static_assert` pins that, because the failure mode is a solid black bar where a hairline or an empty gauge track was drawn, not a subtle shade difference.
- **Red on the NM is semantic, not decorative.** Every NM repaint is a complete tri-color waveform (100% full, measured 93/93), so red is already paid for; it is spent on the attention count, an awaiting state word, and the header rail only while something needs the user. The host simulator renders it: the NM preview compiles under `BOARD_SIM_NM` and never defines `BOARD_NM_EPD_420`, so every red decision was previewed as black until the guards were moved onto the UI-variant macro.
