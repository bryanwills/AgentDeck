...
상용 USB HID 매크로 데크. **Ulanzi Studio 플러그인**(`plugin-ulanzi`, daemon WS 등록 시 `clientType: 'ulanzi-plugin'`)이 **유일한 지원 드라이버** — D200H 는 Mac Ulanzi Studio 앱을 통해야만 안정적으로 렌더된다. **direct-HID 폴백은 retire(2026-06-21, `380c6510`)**: 두 데몬 모두 더 이상 HID 로 D200H 를 열지 않는다. **Node direct-HID 모듈은 삭제됨(2026-07-08)** — `d200h-module.ts` + `bridge/src/d200h/` 제거, `ModuleConfigs.d200h` 플래그도 제거. D200H 연결성은 `ulanzi-plugin` WS presence(`getUlanziClientCount()`)로만 보고. **Swift direct-HID(`D200hHidModule.swift`, `enableD200hDirectHID = false`)는 그대로 dormant 보존.** 출처: [docs/plugin-conventions.md](plugin-conventions.md) §D200H.
...
| **드라이버** | **유일**: Ulanzi Studio 플러그인 `plugin-ulanzi/`(daemon WS). direct-HID: Node 모듈(`bridge/src/d200h/` node-hid)은 **삭제(2026-07-08)**, Swift `D200hHidModule.swift` IOKit 은 **dormant(비활성 보존)**. daemon-only |
...
| **D200H** | ✅ | (참고: 갭 아님) Ulanzi Studio 플러그인이 **단일 드라이버**. direct-HID 폴백 retire(2026-06-21, `380c6510`) — **Node 모듈 삭제(2026-07-08)**, Swift `enableD200hDirectHID=false` dormant 보존 | 최선. 플러그인 없이 HID 직구동 시 화면이 깨져 의도적으로 비활성. Node 재활성은 git history 복원 필요, Swift 는 flip-point 하나 |
