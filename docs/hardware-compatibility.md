...
- LilyGo EPD47 treats a full refresh as a real `epd_clear()` followed by the new grayscale image. It runs before the first frame, on offline/online transitions, after four image-only updates, or after ten minutes; routine changes use the quieter image write. This removes both shipped factory artwork and prior AgentDeck frames that a direct grayscale draw can leave visible beneath the new frame.
- **EPD47 content changes erase the previous frame; only the scheduled pigment reset clears.** One tab tap cost exactly one `epd_clear()`, and one clear on this driver is several whole-panel inversions — a single tap read as a storm of flashes. The canvas retains the frame it last wrote (a second 4-bit PSRAM framebuffer, optional; without it the surface degrades to the clearing path) and a content change drives that frame back to paper with `epd_draw_image(WHITE_ON_WHITE)`, erasing only the pixels carrying ink. The hard waveform is reserved for the anti-ghosting sweep, which resets the retained frame so the next erase does not chase ink that is already gone. Which erase a `full` deserves is decided once in `render()` and passed down.
- **The autonomous tab is hysteretic, because a page change costs a full clear.** `automaticPage()` reads `attention + processing`, a count that oscillates continuously on a working machine, and following it directly made **88% of all EPD47 repaints full clears** against a policy intending 20% (measured 2026-08-30: 713/639, then 754/669 twenty-seven minutes later). `AgentDeckEpd47::arbitratePage` now requires a newly-correct page to hold for two repaint windows before it is adopted, and a settled swap rides the normal cadence instead of forcing an immediate repaint. Attention is exempt and still switches at once. Host-tested in `esp32/sim/tests/`.
