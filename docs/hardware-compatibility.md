...
- **The camera probe also selects the S3-Pro's screen orientation, so the two rows above run different UIs from one binary.** `Camera::init()` runs before display bring-up; a unit that answers comes up **portrait** in the Pocket UI (SESSIONS / CAM / USAGE, LVGL pointer widgets), and a unit that does not comes up **landscape** in the Ticker UI (Focus / Usage / Sessions, rocker + swipe). Both report `t_display_pro` and take the same OTA image, so a board id alone does not tell you which layout a given unit is showing — read the reported orientation, not the board string. The camera unit's stack is portrait-native (panel GRAM, CST226SE touch reporting, shield mounting), which is why that path needs no rotation compensation.
