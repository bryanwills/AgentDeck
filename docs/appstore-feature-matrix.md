...
| Surface Protocol portable Feed negotiation | Partial | Yes | Tier 1 validates Pocket Daily's eight identity headers and negotiates only `feed.pull`, `feed.conditional`, and `glance.read`; its WS stays bounded and its weather-only Feed remains authoritative for that subset. Tier 1 does not grant Outbox execution, pull-OTA staging, or Inbox. Tier 2 owns the full `portable-reader/v1` runtime. This is native local HTTP/WS handling with no subprocess or companion-install prompt. |
