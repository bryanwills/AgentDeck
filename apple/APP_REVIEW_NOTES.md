...
AgentDeck Dashboard is a real-time monitoring and evaluation app for AI coding agents (Claude Code, opt-in Codex lifecycle hooks, opt-in OpenCode local-server monitoring, opt-in Kiro CLI observation read from a user-granted `~/.kiro` folder, and OpenClaw Gateway sessions). The submitted macOS app's built-in Swift daemon owns the dashboard server, hook ingestion, session state, local-network pairing, APME Layer 2 evaluation, and native hardware modules. It shows live status, tool activity, and quality scores on Mac and on the paired iOS companion. When an agent genuinely waits on a question, AgentDeck shows the choices the agent itself supplied. The user may answer from the app, in which case the app replies to that agent's own local hook request with the option the user chose; if nobody answers, the request is released untouched and the agent's normal prompt appears in the user's terminal. AgentDeck never invents a choice and never proceeds on the user's behalf.

**Works standalone on Mac.** The review scope is a clean Mac running only the submitted Swift app. Dashboard, APME reports, the 17-target Device Preview, opt-in Claude/Codex observation, opt-in OpenCode/OpenClaw monitoring, opt-in Kiro observation, and iOS pairing work without another AgentDeck executable. Users start their chosen agent independently and AgentDeck receives only the integrations they enable.
...
- The menu-bar popup includes a compact Activity pane (total active time, per-agent task count/time, and three recent task labels); its **Full report** button opens the detailed Activity tab. Both read the same small local projection of APME task/turn rows, while the full tab additionally shows project, time, tokens, and score. Duration sums agent turn intervals, so time spent by the user reading or typing is not counted as agent work.
- When an independently running same-user CLI daemon owns the local port, the Swift app periodically reads its authenticated loopback-only `/apme/activity` projection. Each tier retains its own SQLite database; the app never opens the CLI database, launches the CLI, scans another container, or rewrites source rows. The disposable JSON cache is deterministically deduplicated with the Swift projection when displayed.
