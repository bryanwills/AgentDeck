...
- **Supported systems**: Windows · macOS (Apple Silicon) · macOS (Intel). Since 1.0.5 the archive carries **no native binary**: the SVG rasterizer is `@resvg/resvg-wasm`, one file identical on every OS and CPU, so the OS matrix is a manifest declaration rather than a set of shipped per-architecture artifacts. (1.0.4 and earlier shipped five `.node` binaries, and macOS Gatekeeper flagged the arm64 one.) Note the manifest's own floor (mac 10.11 / Windows 10) is optimistic — the plugin would load but find no daemon; the real floor is the daemon's, macOS 15 / Windows 11.
