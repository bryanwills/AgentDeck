...
Getting set up
A thin client — it needs the free AgentDeck daemon on the same Mac. Either path works:

    npx @agentdeck/setup

or install the AgentDeck app for Mac, which carries its own daemon and needs no terminal.

The plugin bundles no daemon, touches no USB HID, collects no analytics, and never edits your shell config.
...
설치 방법
얇은 클라이언트라 같은 Mac에서 무료 AgentDeck 데몬이 실행 중이어야 합니다. 둘 중 아무 방법이나 됩니다.

    npx @agentdeck/setup

또는 AgentDeck Mac 앱을 설치하면 앱이 데몬을 내장하고 있어 터미널이 필요 없습니다.

그다음 평소대로 Claude Code, Codex, OpenCode를 실행하면 세션이 키에 나타납니다.

플러그인은 데몬을 내장하지 않으며, USB HID에 직접 접근하거나, 분석 데이터를 수집하거나, 셸 설정을 변경하지 않습니다.
...
セットアップ
シンクライアントのため、同一Mac上で無料のAgentDeckデーモンが動作している必要があります。どちらの方法でも構いません。

    npx @agentdeck/setup

または、デーモンを内蔵したAgentDeck Macアプリをインストールすれば、ターミナルは不要です。

あとは普段どおりClaude Code、Codex、OpenCodeを起動すれば、セッションがキーに表示されます。

プラグインはデーモンを同梱せず、USB HIDへの直接アクセス、解析データの収集、シェル設定の変更は行いません。
