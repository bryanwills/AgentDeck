...
Root [`VERSION`](VERSION) is the repository baseline and compatibility-line anchor, not a patch ceiling and not a runtime negotiation value. It currently reads **1.0.2**, so every maintained target must remain on compatibility line **1.0**. npm/CLI is at `1.0.21`; Stream Deck is at `1.0.6`; Android is at `1.0.10`; Apple is at `1.0.7`; ESP32 is at `1.0.5`; Ulanzi is at `1.0.3`. A target may also advance beyond root's patch without forcing unrelated targets or root to ship. **This sentence is a mirror, not a source** — take the declared values from `pnpm verify-version`, which reads each target's own manifest, and distinguish them from delivered state. Delivered state as of 2026-08-18 — one patch behind the declared values above, because this round has been cut but not yet delivered: Apple `1.0.6` build 5101 is live on macOS and iPhone/iPad, Stream Deck `1.0.5` is published, Ulanzi `1.0.3` is under review, Android Play `1.0.9` (versionCode 11) is live — the public listing reports `1.0.9`, updated 2026-08-15, so the review that was pending on 2026-08-15 has since cleared — and npm `1.0.20` is published (all last verified 2026-08-18 unless noted otherwise).
