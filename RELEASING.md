...
Root [`VERSION`](VERSION) is the repository baseline and compatibility-line anchor, not a patch ceiling and not a runtime negotiation value. It currently reads **1.0.2**, so every maintained target must remain on compatibility line **1.0**. npm/CLI is at `1.0.21`; Stream Deck is at `1.0.6`; Android is at `1.0.10`; Apple is at `1.0.7`; ESP32 is at `1.0.5`; Ulanzi is at `1.0.3`. A target may also advance beyond root's patch without forcing unrelated targets or root to ship. **This sentence is a mirror, not a source** — take the declared values from `pnpm verify-version`, which reads each target's own manifest, and distinguish them from delivered state. Delivered state as of 2026-08-19, re-verified after submission. **Live:** npm `1.0.21` (registry), ESP32 `1.0.5` (GitHub Release, 31 assets), and Elgato Stream Deck `1.0.6` — the product page's embedded `versions` payload reports `status: published`, `publish_date: 2026-08-18T16:46Z`, so the Maker Console's "automatically publish after being approved" toggle did the release. **Approved, propagating:** Apple iOS `1.0.7` build 5201 — App Review shows `심사 완료됨` and the version sits at `배포 준비됨`, while the public product page still serves `1.0.6`. **In review:** Apple macOS `1.0.7` build 5201, and Google Play `1.0.10` (versionCode 12). **Still live meanwhile:** Play `1.0.9` (versionCode 11), Apple macOS `1.0.6` build 5101. **Not cut, and never published:** Ulanzi stays at `1.0.3` with a zero-commit gap. Verified in the portal on 2026-08-19: `ugc.ulanzistudio.com` → Personal Center shows **Works under review: 1** (AgentDeck, update note "1.0.3 is the H5 setup tutorial you asked for on 7 August") and **Published works: 0** — so the D200H plugin has never been live on the Marketplace, and the submission has been open since 2026-08-07. The notification panel carries no reviewer message, so there is nothing to answer; only a follow-up would move it.
...
**The two platforms diverged for four days, then reconverged.** `1.0.5` was submitted on 2026-08-10 with platform-specific CI build numbers and is now live on both platforms:
...
`1.0.2` was approved and published on 2026-07-28, `1.0.3` — the Windows compatibility correction — on 2026-07-31, and `1.0.4` on 2026-08-05. **`1.0.5` was published on 2026-08-10T18:03:56Z** and is now the latest public version at [AgentDeck on the Elgato Marketplace](https://marketplace.elgato.com/product/agentdeck-dce3806b-176e-40f2-be7d-e029bec0f464). The uploaded CI artifact was the `streamdeck-v1.0.5` GitHub Release asset (`SHA-256 5cde49b7f79a2551c04113d1662b59720de3caa7929e2e8072dbcb3e458f6528`), not a locally rebuilt package. The Marketplace's **monotonic-version rule applies** — every subsequent submission needs a version above the latest published version, and same-version resubmission (which pre-publication review revisions allowed) is unavailable once that version is published.
...
`1.0.3` was uploaded to the Ulanzi portal and was verified as **under review** on 2026-08-12. Do not replace it while that review is active.
