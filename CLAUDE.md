...
- **★GNB parity is hand-maintained across five surfaces** — the nav is copy-pasted into `scripts/pages-index.html`, `docs/hardware/index.html`, `tools/creature-simulator/index.html`, `agentdeck-design-system/viewer/index.html`, and the Python heredoc in `scripts/generate-html-report.py`. It has drifted before (Design System lost the GitHub chip, Build Health lost the language select, Build Health sat in its own 1180/24 container). Every surface must expose the same six links + GitHub chip + language select, and share the 1240/32 container. Touching one nav means touching all five; verify by grepping `class="lang"` and `class="gh"` in each. Build Health additionally aliases tokens (`--bg`/`--surface`/`--text`) instead of the canonical names, so `verify-tokens-sync.py` does **not** gate it — check its palette by eye when tokens change.
- **Device photography pipeline**: `scripts/crop-hardware-images.mjs` crops the raw hardware photos into the catalog card frames (STANDARD 1.75:1 = the 349x200 card, WIDE 3.73:1, HERO 3:2). Only the cropped results in `docs/media/` are committed; the multi-MB originals are not, so the task table records which original each shipped image came from. **`.rotate()` must be called with no argument** — that applies the EXIF orientation tag; passing an explicit angle skips it and crops from an unrotated buffer (many iPhone captures are orientation 6: stored 4032x3024, actually 3024x4032). Frame the device body with margin, not just its screen, and verify each crop in a browser before shipping.
