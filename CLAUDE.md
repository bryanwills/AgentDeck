...
- **Site-wide language selection (en/ko/ja)**: overview, Devices, and Live Preview each embed an inline `data-i18n` dictionary + nav `<select id="lang">`; the design-system viewer has its own locale select. All share the `agentdeck-design-locale` localStorage key so one choice follows the visitor across routes. English stays canonical in the DOM (ko/ja override per key); Build Health stays English (CI evidence). Public surface counts (landing "20 surfaces" chips, README table) mirror the `docs/hardware-compatibility.md` surface matrix — update the matrix row first, then the mirrors.
