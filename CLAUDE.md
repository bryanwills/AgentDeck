...
- **plugin/** — Stream Deck SDK v2 plugin. Five actions: `session-slot` (keypad) + four SD+ encoders — E1 Volume (`utility-dial`), E2 Claude Usage (`option-dial`), E3 Codex Usage (`iterm-dial`), E4 Launcher (`launcher`). UUIDs are immutable post-distribution so several no longer match their display name; the mapping is in [docs/v4-layout.md](docs/v4-layout.md). macOS-only, `SDKVersion: 2` (3 is DRM opt-in — see v4-layout before raising it)
